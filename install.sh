...
Next steps:
  ...
  2. Create a Google Cloud "Desktop app" OAuth client (guide in the README),
     click "Download JSON" on it, and save the downloaded file as-is to:
...
install_prebuilt() {
  ...
  # The TAG path (releases/download/<tag>/…), not the "latest release" alias
  # (releases/latest/download/…): the alias resolves to the newest
  # non-prerelease, non-draft release, so it silently 404s if the rolling
  # release is ever marked prerelease — the tag path fetches from the literal
  # `latest` tag regardless.
  base="${IARAGON_RELEASE_BASE:-https://github.com/jeffersonguerin/iaragon/releases/download/latest}"
  ...
  # Point of no return: from the rm -rf on, a failure must ABORT loudly, not
  # fall back. Two reasons: the previous install is already gone (a "success"
  # message would lie), and — the subtle one — this whole function runs as
  # the left side of an `&&` in main(), where POSIX suppresses `set -e`, so
  # without explicit `|| die` a failed cp would sail on and still print
  # "iaragon installed." (reproduced empirically before this guard existed).
  log "installing to $LIBDIR"
  rm -rf "$LIBDIR"
  mkdir -p "$(dirname "$LIBDIR")" || die "cannot create $(dirname "$LIBDIR")"
  cp -a "$reldir" "$LIBDIR" \
    || die "could not copy the release into $LIBDIR — the previous install (if any) was removed; re-run the installer"
  rm -rf "$tmp"; trap - EXIT
  ...
  for name in iaragon iaragon-login iaragon-doctor; do
    cat > "$BINDIR/$name" <<EOF || die "could not write $BINDIR/$name"
#!/bin/sh
# iaragon launcher (generated by install.sh, prebuilt bundle) — self-contained.
exec "$LIBDIR/bin/$name" "\$@"
EOF
    chmod +x "$BINDIR/$name" || die "could not chmod $BINDIR/$name"
  done
